    ...
    <main className="mx-auto w-full max-w-4xl px-6 py-10">
      ...
      <form action={buy}>
        <label className="mt-8 block text-sm">
          ...
          <span className="ml-2 text-xs text-zinc-500">sent to PayCore as the customer — block it under Risk → Blocklist in the dashboard to see a payment refused</span>
          <input name="email" type="email" defaultValue="shopper@acme-goods.test" className="mt-1 block w-full max-w-md rounded-lg border border-zinc-300 bg-white px-3 py-2 dark:border-zinc-700 dark:bg-zinc-900" />
        </label>
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        {PRODUCTS.map((p) => (
          <div key={p.sku} className="flex flex-col rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900">
            ...
            <div className="mt-4 flex items-center justify-between">
              ...
              <button name="sku" value={p.sku} disabled={!configured} className="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-500 disabled:opacity-50">Buy</button>
            </div>
          </div>
        ))}
      </div>
